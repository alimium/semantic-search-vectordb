...
def embed(
    sequence_list: str,
    model: SentenceTransformer = SentenceTransformer("paraphrase-mpnet-base-v2"),
    average: bool = True,
    char_lim: int = 200,
):
    """
    ...
    for s in sequence_list:
        if char_lim is not None and len(s) > char_lim:
            ...
            emb = model.encode(sequence_list)
            embeddings.extend(emb)

    return list(np.mean(embeddings, axis=0, dtype=float)) if average else embeddings
